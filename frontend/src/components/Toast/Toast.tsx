...
const ToastAction = React.forwardRef<React.ElementRef<typeof ToastPrimitives.Action>, React.ComponentPropsWithoutRef<typeof ToastPrimitives.Action>>(
  ({ className, ...props }, ref) => (
    <ToastPrimitives.Action
      ref={ref}
      className={cn(
        'inline-flex h-8 shrink-0 items-center justify-center rounded-md border bg-transparent px-3 text-sm font-medium transition-colors hover:bg-secondary focus:outline-hidden focus:ring-1 focus:ring-ring disabled:pointer-events-none disabled:opacity-50 group-[.negative]:border-muted/40 hover:group-[.negative]:border-negative/30 hover:group-[.negative]:bg-negative hover:group-[.negative]:text-negative-foreground focus:group-[.negative]:ring-negative',
        className
      )}
      {...props}
    />
...
const ToastClose = React.forwardRef<React.ElementRef<typeof ToastPrimitives.Close>, React.ComponentPropsWithoutRef<typeof ToastPrimitives.Close>>(
  ({ className, ...props }, ref) => (
    <ToastPrimitives.Close
      ref={ref}
      className={cn(
        'absolute right-1.5 top-1.5 rounded-md p-1 text-foreground/50 opacity-70 transition-opacity hover:text-foreground focus:opacity-100 focus:outline-hidden focus:ring-1 group-hover:opacity-100 group-[.negative]:text-red-300 hover:group-[.negative]:text-red-50 focus:group-[.negative]:ring-red-400 focus:group-[.negative]:ring-offset-red-600',
        className
      )}
      toast-close=""
      {...props}
