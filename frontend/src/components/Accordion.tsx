...
>(({ className, children, ...props }, ref) => (
  <AccordionPrimitive.Header className="flex">
    <AccordionPrimitive.Trigger
      ref={ref}
      className={cn(
        'flex flex-1 items-center justify-between py-4 font-medium transition-all hover:underline [&[data-state=open]>svg]:rotate-180 data-disabled:text-muted-foreground! data-disabled:no-underline data-disabled:cursor-not-allowed',
        className
      )}
      {...props}
    >
